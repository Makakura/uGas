...
.controller('loginCtrl', function($scope,UserService,$rootScope,$ionicHistory,sharedUtils,$state,$ionicSideMenuDelegate) {
    ...
    $scope.login = function(user) {
      ...
      UserService.getUser({UserName:user.UserName}) // lấy user bằng user name
        .then(function success(data){
            if((data.length>0) && (user.UserName == data[0].UserName) && (user.Pass == data[0].Pass)){
...
.controller('homeCtrl', function($scope,ItemService,$rootScope,$ionicSideMenuDelegate,fireBaseData,$state,$ionicHistory,$firebaseArray,sharedCartService,sharedUtils) {
...
.controller('indexCtrl', function($scope,UserService,$rootScope,sharedUtils,$ionicHistory,$state,$ionicSideMenuDelegate,sharedCartService) {
    $scope.user = UserService.getCurUser();
